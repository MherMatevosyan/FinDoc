...
# Taken from luxemburg ISIN list
dataset_currencies = ['EUR','NOK','GBP','SEK','PEN','RUB','USD','CNY','CAD',
 'AUD','IDR','ZAR','AED','NGN','ZMW','GHS','INR','UGX','TRY','CHF','JPY',
 'ARS','RON','NZD','SKK','MXN','ITL','CZK','BRL','PLN','DKK','HUF','HKD',
 'SGD','DEM','ILS','FRF','CLP','SAR','UYU','MYR','PHP','COP','KZT','RSD','CRC']


# currencies
pycountry_list = [[obj.alpha_3,obj.name] for obj in currencies]
pycountry_set = set(chain.from_iterable(pycountry_list))
forex_list = [[i['cc'],i['symbol']] for i in symbols]
forex_set = set(chain.from_iterable(forex_list))
# letting only non_ascii characters (i.e. currency symbols)
non_ascii_forex = []
for w in forex_set:
    for ch in w:
        non_ascii = False
        if ord(ch)>127:
            non_ascii = True
        if non_ascii:
            non_ascii_forex.append(w)
curr_regex = '|'.join(pycountry_set|set(non_ascii_forex)|set(dataset_currencies))
